document.addEventListener("DOMContentLoaded", () => {
  ...
  const DATA_PATH = "../data/clean-jobs-enriched-csv-enriched.csv";
  ...
  // ======================================================
  // MATRICE DE VALEUR (SCATTER PLOT V2)
  // ======================================================
  ...
  function createSeniorityLadder(data, selector) {
    ...
    groups.forEach((salaries, level) => {
      ...
      if (careerOrder.includes(lvl) && salaries.length >= 5) {
        plotData.push({
          level: lvl,
          median: d3.median(salaries), // Médiane plus robuste que la moyenne ici
        });
      }
    });

    // Tri selon l'ordre de carrière
    plotData.sort(
      (a, b) => careerOrder.indexOf(a.level) - careerOrder.indexOf(b.level)
    );

    // --- Calcul du KPI (Multiplicateur) ---
    ...
    svg
      ...
      .attr("transform", `translate(0,${innerHeight})`)
      .attr("class", "axis")
      // Met la première lettre en majuscule pour l'axe
      .call(
        d3
          .axisBottom(x)
    ...
    svg
      ...
      .attr("y", innerHeight) // Départ du bas pour l'animation
      .attr("height", 0)
